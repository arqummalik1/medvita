...
const roles = [
  { name: 'Patient', description: 'Login to patient portal' },
  { name: 'Doctor', description: 'Login to doctor dashboard' },
]
...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 2. Check and Update Role if needed
      if (data?.user) {
        const selectedRole = role.name.toLowerCase()
        ...
        if (!profileError && profile) {
          if (profile.role !== selectedRole) {
            // Update role to match selection
            ...
            if (updateError) {
              ...
              // Force refresh profile in context to reflect change
      ...
      <div className="max-w-md w-full space-y-8 glass-panel p-10 rounded-3xl relative z-10 shadow-2xl">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-slate-500 dark:text-slate-400">
            Sign in to access your MedVita dashboard
          </p>
        </div>
